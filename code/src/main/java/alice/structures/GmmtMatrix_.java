 ...
 */
public class GmmtMatrix_ extends Matrix_ {
     ...
     */
    public GmmtMatrix_(SparseMatrix_ inMatrix) {
        super(inMatrix);
    }
     ...
     * @param edges
     */
    public GmmtMatrix_(SparseMatrix_ inMatrix, Edge[] edges) {
...
//        System.out.println("NUM Butterflies " + numK22Cliques);
    ...
    public long getNumK22Cliques() {
        return CountingWedges.countWedgesFast(this.getRows(), getNumRows() + 1);
    }
    ...
    protected int getChangeInNumK22Cliques(Edge swappableEdge1, Edge swappableEdge2) {
        ...
        for (int i = 0; i < srcs.length; i++) {
            ...
            // neighbors of src
            int[] neighs = getRowInstance(srcs[i]).values;
            for (int h : neighs) {
                int[] hneighs = getColInstance(h).values;
                for (int v : hneighs) {
                    if (v == srcs[i]) {
                        continue;
                    }
            ...
            int[] NewNeigh = getColInstance(dsts[(i + 1) % 2]).values;
            for (int v : NewNeigh) {
