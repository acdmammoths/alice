...
public class RefinedMatrix extends GmmtMatrix {
    ...
    public Set<Integer> getEqRowSumRows(int rowSum) {
        return this.rowSumToEqRowSumRows.getOrDefault(rowSum, new HashSet<>());
    }

    public void addEqRowSumRow(int rowSum, int row) {
        final Set<Integer> rows = this.getEqRowSumRows(rowSum);
        rows.add(row);
        this.rowSumToEqRowSumRows.put(rowSum, rows);
    }

    public int getNumEntriesNeq(int row1, int row2) {
        return getMatrix().getNumEntriesNeq(row1, row2);
    }

    /**
     * Gets (1) the degree of the matrix in the Markov chain and (2) the total
     * number of swappable pairs of edges for the matrix that don't include
     * swapping to an equivalent "adjacent" matrix. We compute both quantities
     * in one method because computing the number of K22 cliques, which is
     * needed to compute the degree, requires doing the same work as computing
     * the number of equivalent adjacent matrices, which is needed to compute
     * the total number of swappable pairs of edges. Specifically, the work that
     * is needed in both computations is the nested for loops iterating over the
     * rows. Reference: (1) Gionis et al., Theorem 4.3, Equation (2) Theorem 5.5
     * in the paper.
     *
    ...
    /**
     * Gets the total number of swappable pairs of edges for the adjacent matrix
     * defined by the swappable edges that don't include swapping to an
     * equivalent matrix.
     *
     ...
     * @param totalNumSwapPairs the total number of swappable pairs for the
     * current matrix
    ...
    /**
     * Gets the change in the number of equivalent adjacent matrices for
     * transitioning from the current matrix to the adjacent matrix defined by
     * the swappable edges. Reference: Algorithm 1 in the paper.
     *
     * @param swappableEdge1 the first swappable edge
     * @param swappableEdge2 the second swappable edge
     * @param newEdge1 the first new edge to be added
     * @param newEdge2 the second new edge to be added
     * @param updatedRowPairToNumEntriesNeqPairs an empty map that the updated
     * row pair to number of entries not equal pairs are added to
     * @return the change in the number of equivalent adjacent matrices
     */
    public int getChangeInNumEquivAdjMatrices(
    ...
    /**
     * Gets the correction factor for the number of swappable pairs of edges
     * that transition from the matrix to the adjacent matrix defined by the
     * input rows. Note that the correction factor is the same for the number of
     * swappable pairs of edges that transition from the adjacent matrix to the
    ...
    /**
     * Gets the number of swappable pairs of edges that transition from the
     * matrix to the adjacent matrix defined by the swappable rows. Reference:
     * Lemma 5.3 in the paper.
     *
     ...
     * @return the number of swappable pairs of edges that transition from the
     * current matrix to the adjacent matrix
     */
    ...
    /**
     * Gets the number of swappable pairs of edges that transition from the
     * adjacent matrix defined by the new rows to the current matrix. Reference:
     * Corollary 5.4 in the paper.
     *
     * @param numSwapPairsFactor the correction factor
     * @param newRow1 the row that contains the first new edge
     * @param newRow2 the row that contains the second new edge
     * @return the number of swappable pairs of edges that transition from the
     * adjacent matrix to the current matrix
     */
    ...
    /**
     * Determines whether all entries are equal across two rows, excluding the
     * entries in the specified skipped columns.
     *
     ...
     */
    public boolean entriesEqual(int row1, int row2, int skipCol1, int skipCol2) {
        return getMatrix().entriesEqual(row1, row2, skipCol1, skipCol2);
    }

    /**
     * Gets the swappable edges and new edges that will potentially be added to
     * the list of edges for the matrix for the Refined chain. Note that this
     * method only returns edges that lead to a matrix that does not represent
     * the same dataset.
     *
